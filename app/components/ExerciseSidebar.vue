    ...
    <nav v-if="!collapsed" class="ex-list" aria-label="Exercises">
      ...
      >
        <div class="ex-group-header">
          <span
            class="ex-cat-dot"
            :style="{ background: CAT_COLOR[group.category] }"
          />
          <span
        ...
        >
          ...
          <button class="ex-item-header" @click="toggleItem(item.id)">
            ...
            <span class="ex-chevron" :class="{ 'is-open': isExpanded(item.id) }"
              >›</span
            >
          </button>
...
.ex-toggle:hover {
  background: rgba(156, 121, 72, 0.06);
}
...
.ex-toggle:hover .ex-toggle-icon {
  color: var(--c-gold);
}
...
.ex-item.is-open {
  background: rgba(156, 121, 72, 0.05);
}
...
.ex-item-header:hover {
  background: rgba(156, 121, 72, 0.06);
}
.ex-item.is-active .ex-item-header {
...
.ex-body-leave-active {
  transition:
    opacity 200ms ease,
    max-height 220ms cubic-bezier(0.65, 0, 0.35, 1);
  max-height: 120px;
  overflow: hidden;
}
.ex-body-enter-from,
